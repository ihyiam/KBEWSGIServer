...
if py3k:
    ...
    urlunquote = functools.partial(urlunquote, encoding='utf-8')
...
class Bottle(object):
    ...
    def run(self, **kwargs):
        ...
        webmgr = kwargs.get('webmgr',None)
        if webmgr is not None and self.webmgr is None:
            self.webmgr = webmgr
        run(self, **kwargs)
    ...
    def _handle(self, environ):
        DEBUG_MSG("URI: %s?%s , from:%s"%(environ.get('PATH_INFO',''),environ.get('QUERY_STRING',''),environ.get('REMOTE_ADDR','')))
    ...
    def wsgi(self, environ, start_response):
        ...
        try:
            ...
            if not self.catchall: raise
            err = '<h1>Critical errors while processing request: %s</h1>' \
    ...
    def apply(self, callback, route):
        ...
        def wrapper(*a, **ka):
            ...
            if isinstance(rv, dict):
                ...
                rv.content_type = 'application/json; charset=UTF-8'
            return rv
...
class WSGIRefServer(ServerAdapter):
    def run(self, app): # pragma: no cover
        ...
        from KBEWSGIServer import KBEWSGIServer,KBEFixedHandler
        import socket


        handler_cls = self.options.get('handler_class', KBEFixedHandler)
        server_cls  = self.options.get('server_class', KBEWSGIServer)
        webmgr = self.options.get('webmgr', None)
        ...
        srv = server_cls((self.host, self.port), handler_cls , webmgr)
        srv.set_app(app)
        app.server = srv
        # srv.serve_forever()
